...
import CategoryNavMenu from "@/components/layout/CategoryNavMenu";
              ...
              {navLinks.map((link) =>
                link.hasDropdown ? (
                  ...
                  >
                    ...
                    {catOpen ? (
                      <div className="absolute left-1/2 top-full z-50 -mt-1 w-max -translate-x-1/2 pt-2">
                        <CategoryNavMenu items={categoryNav} />
                      </div>
                    ) : null}
                  </li>
                ) : (
        ...
        {isOpen && (
          <div className="overflow-hidden border-t border-border lg:hidden">
            <div className="px-4 py-4">
              ...
              {navLinks.map((link) =>
                link.hasDropdown ? (
                  <div key={link.label} className="border-b border-border/60 last:border-0">
                    <button
                      type="button"
                      className="flex w-full items-center justify-between rounded-lg py-3 font-semibold"
                      onClick={() => setMobileCatOpen((open) => !open)}
                      aria-expanded={mobileCatOpen}
                    >
                      {link.label}
                      <ChevronDown
                        className={cn("h-5 w-5 transition-transform", mobileCatOpen && "rotate-180")}
                      />
                    </button>
                    {mobileCatOpen ? (
                      <div className="pb-3">
